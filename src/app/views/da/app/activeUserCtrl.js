...
angular.module('app.controller').controller('activeUserCtrl', [
    ...
    function($scope, chartOption, serviceAPI, urlAPI, msgService) {
        ...
        $scope.rankChart = function() {
            ...
            $scope.getEchData(urlAPI.report_app_activeuser_modelRank, 'modelrank', function(result) {
                var option = $scope.getBarOption(result, 'model');
                $scope.modelrankChart.setOption(option);
            });
            $scope.getEchData(urlAPI.report_app_activeuser_regionRank, 'regionrank', function(result) {
