...
angular.module('app.controller').controller('notifCreatveCtrl', [
    "$scope", "serviceAPI", "Upload", 'urlAPI', '$sce', "$anchorScroll", "$location", "toaster", "SweetAlert",
    function($scope, serviceAPI, Upload, urlAPI, $sce, $anchorScroll, $location, toaster, SweetAlert) {
        $scope.option = 0;
        ...
        $scope.removeMessage = function(index) {
            SweetAlert.swal({
                title: "Are you sure?",
                text: "Sure to delete  message " + $scope.notification[index].order,
                type: "warning",
        ...
        $scope.uploadPic = function(file) {
            if (file) {
                if (file && file.size >= 100000) {
                    ...
                    toaster.warning({ body: "Wrong Format", timeout: 2500 });
                    return false;
                } else {
                    Upload.upload({
                        ...
                        toaster.success({ body: "Upload Succeeded" });
        ...
        $scope.checkNum = function(num) {
            if (isNaN(Number($scope.receiver.triggerTime)) || Number($scope.receiver.triggerTime) < 0 || $scope.receiver.triggerTime == '') {
                ...
                toaster.warning({ body: "Please enter a number more then 0" });
            }
        ...
        $scope.saveDetail = function(num) {
            ...
            if (num == 0) {
                ...
                if ($scope.flag) {
                    ...
                    serviceAPI.saveData(url, receiver).then(function(result) {
                        if (result.status == 1 && result.code == 200) {
                            $scope.nextStep(3, 'view.pushDetail.step3');
                        } else {
                            toaster.warning({ body: result.msg });
                        }
                    })
